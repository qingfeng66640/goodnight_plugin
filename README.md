...
# 允许触发主动晚安氛围提醒的群聊。
# 格式："平台:群号"
# OneBot/QQ 场景的平台名仍是 "qq"，不要写成 "onebot:群号"。
...
这里的 `平台` 匹配运行时消息里的 `Message.platform`，不是 adapter 插件名。Neo-MoFox 1.2.0-rc 将 `napcat_adapter` 改名为 `onebot_adapter` 后，OneBot/QQ 消息的平台名仍是 `qq`；如果写成 `onebot:群号`，白名单不会命中，对应的晚安或劝导提醒也不会触发。
